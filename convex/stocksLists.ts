...
// ============================================
// ACTIONS (for server-side access)
// ============================================

/**
 * Action: Get all stocks in the active list (for server-side access)
 * This can be called from API routes with userId
 */
export const getListStocksAction = action({
  args: { userId: v.string() },
  handler: async (ctx, args): Promise<Array<{ id: string; symbol: string; addedAt: number }>> => {
    const activeList = await ctx.runQuery(api.stocksLists.getActiveListForUser, {
      userId: args.userId,
    });
    ...
    const stocks = await ctx.runQuery(api.stocksLists.getStocksForList, {
...
/**
 * Internal query: Get active list for a specific user (for server-side access)
 */
export const getActiveListForUser = internalQuery({
  args: { userId: v.string() },
...
/**
 * Internal query: Get stocks for a specific list (for server-side access)
 */
export const getStocksForList = internalQuery({
  args: { listId: v.id("stocksLists") },
  handler: async (ctx, args) => {
    const stocks = await ctx.db
      .query("listStocks")
