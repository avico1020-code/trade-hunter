 ...
 */
async function checkIBGatewayConnection(): Promise<{
  connected: boolean;
  accountType?: string;
  error?: string;
  port?: number;
  ...
  try {
    ...
    // Log for debugging
    console.log(
      `[IBKR Status Check] Checking IbkrConnectionManager on port ${status.port}, state: ${status.state}`
    );
    ...
    if (connectionManager.isConnected()) {
      ...
      console.log(
        `[IBKR Status Check] ✅ Connection found on port ${status.port} (${status.state})`
      );
    ...
    console.log(
      `[IBKR Status Check] ⚠️ Connection manager exists but not connected (state: ${status.state}, port: ${status.port})`
    );
    if (status.lastError) {
    ...
    return {
      ...
      error:
        status.lastError ||
        `Not connected (state: ${status.state}). Connection will be established when market data is requested.`,
      port: status.port,
    };
  } catch (err) {
      ...
      return NextResponse.json(
        {
          ...
          connected: false,
          message:
            "Not connected to IB Gateway (connection will be established when market data is requested)",
