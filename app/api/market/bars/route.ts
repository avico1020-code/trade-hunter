...
export async function GET(request: NextRequest) {
  try {
    ...
    // Try IBKR first (via MarketDataHub)
    const hub = getMarketDataHub();
    const ibkrBars = hub.getBars(symbol.toUpperCase(), timeframe as any);

    console.log(`[Market Bars API] IBKR returned ${ibkrBars.length} bars`);
    ...
    try {
      ...
      const yahooBars = await fetchYahooFinanceBars(symbol, interval, range);
