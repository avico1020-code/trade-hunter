...
export function CandlestickChart({ symbol, timeframe }: CandlestickChartProps) {
  ...
  useEffect(() => {
    ...
    const initChart = async () => {
      try {
        ...
        // Add candlestick series (v5.x API - using addSeries with CandlestickSeries)
        ...
        // CRITICAL: Validate series was created
        if (!series) {
  ...
  useEffect(() => {
    ...
    const tryLoadData = async () => {
      ...
      // Final validation - just check that series exists
      if (!seriesRef.current || !chartRef.current) {
        console.error(`[CandlestickChart] ❌ Series or chart not available`);
        setDataError("Chart not properly initialized");
        return;
      }
      ...
      const loadAndSubscribe = async () => {
        try {
          ...
          // Series is already validated - it was created with CandlestickSeries
          ...
          try {
            ...
            console.log(`[CandlestickChart] Series ready for data`);
