...
export class IbkrAccountService {
  ...
  async getAccountSummary(accountId: string = ""): Promise<AccountSummary> {
    ...
    return new Promise((resolve, reject) => {
      ...
      const onAccountSummary = (
        ...
        console.log(
          `[Account Service] 📥 Received accountSummary event: reqId=${reqId}, account=${account}, tag=${tag}, value=${value}, currency=${currency}`
        );
        if (account !== accountId && accountId !== "") {
          console.log(
            `[Account Service] ⏭️ Skipping account ${account} (requested: ${accountId || "all"})`
          );
          return;
        }
        ...
        if (
          summary.netLiquidation !== null &&
          summary.netLiquidation !== undefined &&
          summary.availableFunds !== null &&
          summary.availableFunds !== undefined
        ) {
      ...
      const onAccountSummaryEnd = (reqId: number) => {
        console.log(
          `[Account Service] ✅ accountSummaryEnd received: reqId=${reqId}, fieldsReceived=${fieldsReceived}`
        );
        ...
        if (fieldsReceived === 0) {
          console.log(
            `[Account Service] ❌ No fields received for ${accountId || "all accounts"}`
          );
      ...
      // Set up listeners
      // Use 'on' for accountSummary because it fires multiple times (once per tag)
      (client as any).on("accountSummary", onAccountSummary);
  ...
  async subscribeAccountUpdates(accountId: string = ""): Promise<void> {
    ...
    try {
      ...
      client.on("accountValue", (...args: unknown[]) => {
        const [key, val, currency, accountName] = args as [string, string, string, string];
