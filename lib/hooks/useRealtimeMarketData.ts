...
export function useRealtimeMarketData(symbol: string | null): MarketDataResult {
  const data = useQuery(api.marketData.getMarketData, symbol ? { symbol } : "skip");
  ...
  const fetchData = useCallback(
    async (sym: string) => {
        ...
        if (statusRes.ok) {
          ...
          if (ibkrConnected) {
            if (ibkrDisabledUntil.current > 0 || ibkrFailureCount.current > 0) {
              console.log(
                `✅ [useRealtimeMarketData] IB Gateway connected - resetting circuit breaker for ${sym}`
              );
      ...
      if (!ibkrConnected && now < ibkrDisabledUntil.current) {
        ...
        console.log(
          `⏸️ [useRealtimeMarketData] IBKR disabled for ${sym} (${minutesLeft}m remaining) - using Yahoo Finance directly`
        );
        try {
      ...
      try {
        ...
        if (ibkrConnected) {
          console.log(
            `🔍 [useRealtimeMarketData] Fetching ${sym} from IBKR (IB Gateway connected)...`
          );
        } else {
        ...
        try {
          ...
          if (
            ...
            console.warn(
              `⚠️ [useRealtimeMarketData] IBKR request timeout for ${sym} - falling back to Yahoo Finance`
            );
            ...
            if (ibkrFailureCount.current >= 5) {
              ...
              console.warn(
                `🚫 [useRealtimeMarketData] IBKR disabled for 10 minutes after ${ibkrFailureCount.current} failures`
              );
            } else if (ibkrFailureCount.current >= 3) {
              ibkrDisabledUntil.current = now + 5 * 60 * 1000; // 5 minutes
              console.warn(
                `🚫 [useRealtimeMarketData] IBKR disabled for 5 minutes after ${ibkrFailureCount.current} failures`
              );
            }
            // Don't throw - fall through to Yahoo Finance fallback
            await fetchYahoo({ symbol: sym }).catch((err) => {
              console.error(
                `❌ [useRealtimeMarketData] Yahoo Finance fallback also failed for ${sym}:`,
                err
              );
            });
            return;
        ...
        if (ibkrRes.ok) {
          ...
          if (lastPrice > 0 && closePrice > 0) {
            ...
            console.log(
              `✅ [useRealtimeMarketData] IBKR SUCCESS for ${sym}: $${lastPrice.toFixed(2)} (${changePercent >= 0 ? "+" : ""}${changePercent.toFixed(2)}%)`
            );
            return;
          } else {
            console.warn(
              `⚠️ [useRealtimeMarketData] IBKR returned invalid data for ${sym} (price: ${lastPrice}, close: ${closePrice})`
            );
          }
        } else {
          const errorText = await ibkrRes.text().catch(() => "");
          console.warn(
            `⚠️ [useRealtimeMarketData] IBKR HTTP ${ibkrRes.status} for ${sym}: ${errorText.substring(0, 100)}`
          );
          ...
          if (ibkrRes.status === 503) {
            ...
            if (ibkrFailureCount.current >= 5) {
              ibkrDisabledUntil.current = now + 10 * 60 * 1000; // 10 minutes
              console.warn(
                `🚫 [useRealtimeMarketData] IBKR disabled for 10 minutes after ${ibkrFailureCount.current} consecutive failures`
              );
            } else if (ibkrFailureCount.current >= 3) {
              ibkrDisabledUntil.current = now + 5 * 60 * 1000; // 5 minutes
              console.warn(
                `🚫 [useRealtimeMarketData] IBKR disabled for 5 minutes after ${ibkrFailureCount.current} consecutive failures`
              );
            }
          }
        }
        ...
        console.warn(
          `⚠️ [useRealtimeMarketData] IBKR FAILED for ${sym}, falling back to Yahoo Finance...`
        );
        await fetchYahoo({ symbol: sym });
      } catch (error) {
        ...
        try {
          ...
          console.error(
            `❌ [useRealtimeMarketData] Yahoo Finance also failed for ${sym}:`,
            yahooError
          );
        }
      }
  ...
  useEffect(() => {
    ...
    if (dataAge > STALE_THRESHOLD) {
      console.log(
        `🔄 [useRealtimeMarketData] ${symbol} data is stale (${Math.floor(dataAge / 1000)}s old), refreshing from streaming...`
      );
      fetchData(symbol);
    }
