 ...
 */
export function calculateVolumeMultiple(
  candle: Candle,
 ...
 */
function calculateUpTrendStrength(
  candles: Candle[],
  index: number,
  lookback: number = 8
): number {
 ...
 */
function calculateHighDistancePct(candle: Candle, parts: ReturnType<typeof getCandleParts>): number {
  if (parts.totalRange <= 0) return 0;
 ...
 */
function calculateClosePosPctFromHigh(candle: Candle, parts: ReturnType<typeof getCandleParts>): number {
  if (parts.totalRange <= 0) return 0;
...
export function detectBullishHammer(context: PatternContext): CandlePattern | null {
  ...
  const distanceToSupport = context.nearestSupport !== undefined
  ...
  } else if (bodyTopPct >= 0.50 && bodyTopPct < 0.65) {
    topScore = 0.25; // weak
  } else if (bodyTopPct >= 0.65 && bodyTopPct < 0.80) {
    topScore = 0.50; // medium
  } else if (bodyTopPct >= 0.80) {
    topScore = 1.0; // strong
  }
  ...
  let srScore = 0.0;
  if (distanceToSupport >= 0.50 && distanceToSupport <= 1.0) {
    srScore = 0.25; // weak
  } else if (distanceToSupport >= 0.25 && distanceToSupport < 0.50) {
    srScore = 0.50; // medium
  ...
  const strength = calculateWeightedStrength([
    ...
    { score: bodyScore, weight: 0.20 },
    { score: topScore, weight: 0.15 },
    { score: trendScore, weight: 0.10 },
    { score: volScore, weight: 0.10 },
    { score: srScore, weight: 0.10 },
  ]);
...
export function detectShootingStar(context: PatternContext): CandlePattern | null {
  ...
  const distanceToResistance = context.nearestResistance !== undefined
  ...
  let bottomScore = 0.0;
  if (bodyBottomPct > 0.50) {
    bottomScore = 0.0; // invalid
  } else if (bodyBottomPct >= 0.35 && bodyBottomPct <= 0.50) {
    bottomScore = 0.25; // weak
  } else if (bodyBottomPct >= 0.20 && bodyBottomPct < 0.35) {
    bottomScore = 0.50; // medium
  } else if (bodyBottomPct < 0.20) {
  ...
  let srScore = 0.0;
  if (distanceToResistance >= 0.50 && distanceToResistance <= 1.0) {
    srScore = 0.25; // weak
  } else if (distanceToResistance >= 0.25 && distanceToResistance < 0.50) {
    srScore = 0.50; // medium
...
export function detectBullishEngulfing(context: PatternContext): CandlePattern | null {
  ...
  const dominanceRatio = bodyPrev + bodyCurr > 0 
    ? bodyCurr / (bodyCurr + bodyPrev)
    : 0;
  ...
  if (!engulfingConditionsMet) {
    return {
      ...
      metrics: {
        ...
        volumeMultiple: calculateVolumeMultiple(currCandle, context.volumeAverage ?? calculateAverageVolume(candles, index, 20)),
        downTrendStrengthPct: calculateDownTrendStrength(candles, index, 8),
        distanceToSupportPct: context.nearestSupport !== undefined 
          ? Math.abs(currCandle.close - context.nearestSupport) / currCandle.close * 100 
          : 100.0,
        ...
        distanceToSRpct: context.nearestSupport !== undefined 
          ? Math.abs(currCandle.close - context.nearestSupport) / currCandle.close * 100 
          : 100.0,
      },
    };
  ...
  if (bodyCurrRangeRatio < 0.40) {
    bodyDominanceScore = 0.25; // weak
  } else if (bodyCurrRangeRatio >= 0.40 && bodyCurrRangeRatio < 0.55) {
    bodyDominanceScore = 0.50; // medium
  } else if (bodyCurrRangeRatio >= 0.55 && bodyCurrRangeRatio <= 0.70) {
    bodyDominanceScore = 0.75; // strong
  } else if (bodyCurrRangeRatio > 0.70) {
    bodyDominanceScore = 1.0; // very strong
  }
  
  // C) Dominance Ratio Score (dominanceRatioScore)
  let dominanceRatioScore = 0.0;
  if (dominanceRatio >= 0.50 && dominanceRatio < 0.60) {
    dominanceRatioScore = 0.25; // weak
  } else if (dominanceRatio >= 0.60 && dominanceRatio < 0.70) {
    dominanceRatioScore = 0.50; // medium
  } else if (dominanceRatio >= 0.70 && dominanceRatio <= 0.80) {
    dominanceRatioScore = 0.75; // strong
  } else if (dominanceRatio > 0.80) {
  ...
  // =========================
  const strength = calculateWeightedStrength([
    { score: engulfScore, weight: 0.30 },
    { score: bodyDominanceScore, weight: 0.20 },
    { score: dominanceRatioScore, weight: 0.15 },
    { score: trendScore, weight: 0.15 },
    { score: volumeScore, weight: 0.10 },
    { score: srScore, weight: 0.10 },
  ]);
  ...
  if (!engulfingConditionsMet) {
    return {
      ...
      metrics: {
        ...
        distanceToResistancePct: context.nearestResistance !== undefined
          ? Math.abs(context.nearestResistance - currCandle.close) / currCandle.close * 100
          : 100.0,
        bodyRatio: bodyCurrRatio,
        lowerWickRatio: 0,
        ...
        distanceToSRpct: context.nearestResistance !== undefined
          ? Math.abs(context.nearestResistance - currCandle.close) / currCandle.close * 100
          : 100.0,
      },
    };
...
export function detectHangingMan(context: PatternContext): CandlePattern | null {
  ...
  const lowerWickValid = lowerWickRatio >= 0.40;
  const bodyRatioValid = bodyRatio <= 0.35;
  const bodyTopValid = bodyTopPct >= 0.60;
  ...
  // A) Lower Wick Ratio Quality
  let wickScore = 0.0;
  if (lowerWickRatio >= 0.40 && lowerWickRatio < 0.50) {
    wickScore = 0.25; // weak
  } else if (lowerWickRatio >= 0.50 && lowerWickRatio < 0.65) {
    wickScore = 0.50; // medium
  } else if (lowerWickRatio >= 0.65 && lowerWickRatio <= 0.80) {
    wickScore = 0.75; // strong
  } else if (lowerWickRatio > 0.80) {
  ...
  if (bodyTopPct >= 0.60 && bodyTopPct < 0.70) {
    bodyTopScore = 0.25; // weak
  } else if (bodyTopPct >= 0.70 && bodyTopPct < 0.85) {
    bodyTopScore = 0.50; // medium
...
export function detectInvertedHammer(context: PatternContext): CandlePattern | null {
  ...
  // Basic requirements check
  const upperWickValid = upperWickRatio >= 0.50;
  const bodyRatioValid = bodyRatio <= 0.35;
  const bodyBottomValid = bodyBottomPct <= 0.40;
  ...
  // A) Upper Wick Ratio Quality
  let upperWickScore = 0.0;
  if (upperWickRatio >= 0.50 && upperWickRatio < 0.65) {
    upperWickScore = 0.25; // weak
  } else if (upperWickRatio >= 0.65 && upperWickRatio < 0.80) {
    upperWickScore = 0.50; // medium
  } else if (upperWickRatio >= 0.80 && upperWickRatio <= 1.00) {
    upperWickScore = 0.75; // strong
  } else if (upperWickRatio > 1.00) {
  ...
  if (bodyBottomPct > 0.50) {
    ...
    bodyBottomScore = 0.50; // medium
  } else if (bodyBottomPct < 0.20) {
    bodyBottomScore = Math.min(1.0, 0.75 + (0.20 - bodyBottomPct) * 5.0); // strong, cap at 1.0
  }
...
export function detectDragonflyDoji(context: PatternContext): CandlePattern | null {
  ...
  const upperWickValid = upperWickRatio <= 0.20;
  const lowerWickValid = lowerWickRatio >= 0.60;
  ...
  if (bodyRatio > 0.15) {
    bodyScore = 0.0; // invalid
  } else if (bodyRatio >= 0.10 && bodyRatio <= 0.15) {
    bodyScore = 0.25; // weak
  } else if (bodyRatio >= 0.05 && bodyRatio < 0.10) {
    bodyScore = 0.50; // medium
  ...
  let upperWickScore = 0.0;
  if (upperWickRatio > 0.20) {
    upperWickScore = 0.0; // invalid
  } else if (upperWickRatio >= 0.10 && upperWickRatio <= 0.20) {
    upperWickScore = 0.25; // weak
  } else if (upperWickRatio >= 0.05 && upperWickRatio < 0.10) {
    upperWickScore = 0.50; // medium
  } else if (upperWickRatio < 0.05) {
    upperWickScore = 1.0; // strong
  }
  ...
  } else if (lowerWickRatio >= 0.60 && lowerWickRatio < 0.70) {
    lowerWickScore = 0.25; // weak
  } else if (lowerWickRatio >= 0.70 && lowerWickRatio < 0.80) {
    lowerWickScore = 0.50; // medium
  } else if (lowerWickRatio >= 0.80 && lowerWickRatio <= 0.90) {
    lowerWickScore = 0.75; // strong
  } else if (lowerWickRatio > 0.90) {
  ...
  const strength = calculateWeightedStrength([
    ...
    { score: bodyScore, weight: 0.15 },
    { score: upperWickScore, weight: 0.10 },
    { score: highPosScore, weight: 0.10 },
    { score: trendScore, weight: 0.15 },
    { score: volumeScore, weight: 0.10 },
...
export function detectGravestoneDoji(context: PatternContext): CandlePattern | null {
  ...
  // Basic requirements check
  const bodyRatioValid = bodyRatio <= 0.15;
  const lowerWickValid = lowerWickRatio <= 0.20;
  const upperWickValid = upperWickRatio >= 0.60;
  ...
  let lowerWickScore = 0.0;
  if (lowerWickRatio > 0.20) {
    lowerWickScore = 0.0; // invalid
  } else if (lowerWickRatio >= 0.10 && lowerWickRatio <= 0.20) {
    lowerWickScore = 0.25; // weak
  } else if (lowerWickRatio >= 0.05 && lowerWickRatio < 0.10) {
    lowerWickScore = 0.50; // medium
  } else if (lowerWickRatio < 0.05) {
    lowerWickScore = 1.0; // strong
  }
  ...
  if (upperWickRatio < 0.60) {
    upperWickScore = 0.0; // invalid
  } else if (upperWickRatio >= 0.60 && upperWickRatio < 0.70) {
    upperWickScore = 0.25; // weak
  } else if (upperWickRatio >= 0.70 && upperWickRatio < 0.80) {
    upperWickScore = 0.50; // medium
  } else if (upperWickRatio >= 0.80 && upperWickRatio <= 0.90) {
    upperWickScore = 0.75; // strong
  } else if (upperWickRatio > 0.90) {
    upperWickScore = 1.0; // very strong
  }
  ...
  const strength = calculateWeightedStrength([
    { score: upperWickScore, weight: 0.25 },
    { score: bodyScore, weight: 0.15 },
    { score: lowerWickScore, weight: 0.10 },
    { score: lowPosScore, weight: 0.10 },
    { score: trendScore, weight: 0.15 },
    { score: volumeScore, weight: 0.10 },
    { score: srScore, weight: 0.15 },
  ]);
...
export function detectSpinningTop(context: PatternContext): CandlePattern | null {
  ...
  const wickBalance = wickTotalRatio > 0
    ? 1 - Math.abs(upperWickRatio - lowerWickRatio) / wickTotalRatio
    : 0;
  ...
  // Basic requirements check
  const bodyRatioValid = bodyRatio >= 0.10 && bodyRatio <= 0.45;
  const upperWickValid = upperWickRatio >= 0.20;
  const lowerWickValid = lowerWickRatio >= 0.20;
  ...
  // A) Body Ratio Quality
  let bodyScore = 0.0;
  if (bodyRatio < 0.10 || bodyRatio > 0.45) {
    bodyScore = 0.0; // invalid
  } else if (bodyRatio >= 0.10 && bodyRatio < 0.15) {
    bodyScore = 0.25; // weak
  } else if (bodyRatio >= 0.15 && bodyRatio < 0.25) {
    ...
    bodyScore = 0.50; // moderate (leaning to small candle but slightly stronger body)
  }
  
  // B) Wick Total Ratio Quality
  let wickTotalScore = 0.0;
  if (wickTotalRatio < 0.40) {
    wickTotalScore = 0.0; // invalid
  } else if (wickTotalRatio >= 0.40 && wickTotalRatio < 0.60) {
    wickTotalScore = 0.25; // weak
  } else if (wickTotalRatio >= 0.60 && wickTotalRatio < 0.80) {
    wickTotalScore = 0.50; // medium
  } else if (wickTotalRatio >= 0.80 && wickTotalRatio <= 1.00) {
    wickTotalScore = 0.75; // strong
  } else if (wickTotalRatio > 1.00) {
    wickTotalScore = 1.0; // very strong
  }
  
  // C) Wick Balance Quality
  let wickBalanceScore = 0.0;
  if (wickBalance >= 0.0 && wickBalance < 0.40) {
    wickBalanceScore = 0.25; // poor balance
  } else if (wickBalance >= 0.40 && wickBalance < 0.70) {
    wickBalanceScore = 0.50; // medium
  } else if (wickBalance >= 0.70 && wickBalance <= 0.90) {
    wickBalanceScore = 0.75; // strong
  } else if (wickBalance > 0.90) {
  ...
  if (trendStrengthPct >= 0 && trendStrengthPct < 2.0) {
    ...
    trendScore = 0.50; // moderate context
  ...
  const strength = calculateWeightedStrength([
    ...
    { score: wickBalanceScore, weight: 0.20 },
...
export function detectHighWave(context: PatternContext): CandlePattern | null {
  ...
  const wickBalance = wickTotalRatio > epsilon
    ? 1 - Math.abs(upperWickRatio - lowerWickRatio) / wickTotalRatio
    : 0;
  ...
  // Basic requirements check
  const bodyRatioValid = bodyRatio <= 0.35;
  const wickTotalValid = wickTotalRatio >= 0.60;
  ...
  // B) Wick Total Ratio Quality
  let wickTotalScore = 0.0;
  if (wickTotalRatio < 0.60) {
    wickTotalScore = 0.0; // invalid
  } else if (wickTotalRatio >= 0.60 && wickTotalRatio < 0.80) {
    wickTotalScore = 0.25; // weak
  } else if (wickTotalRatio >= 0.80 && wickTotalRatio < 1.00) {
    wickTotalScore = 0.50; // medium
  } else if (wickTotalRatio >= 1.00 && wickTotalRatio <= 1.20) {
    wickTotalScore = 0.75; // strong
  } else if (wickTotalRatio > 1.20) {
    wickTotalScore = 1.0; // very strong
  }
  ...
  if (trendStrengthPct >= 0 && trendStrengthPct < 2.0) {
    ...
    trendScore = 0.50; // moderate trend
  } else if (trendStrengthPct >= 5.0) {
    trendScore = Math.min(1.0, 0.75 + (trendStrengthPct - 5.0) / 10.0); // strong trend context, cap at 1.0
  }
  ...
  const strength = calculateWeightedStrength([
    { score: bodyScore, weight: 0.20 },
    { score: wickTotalScore, weight: 0.25 },
    { score: wickBalanceScore, weight: 0.10 },
    { score: atrScore, weight: 0.20 },
    { score: volumeScore, weight: 0.15 },
    { score: trendScore, weight: 0.10 },
  ]);
...
export function detectStandardDoji(context: PatternContext): CandlePattern | null {
  ...
  if (wickBalance >= 0.0 && wickBalance < 0.40) {
    ...
    wickBalanceScore = 0.50; // medium balance
  } else if (wickBalance >= 0.70 && wickBalance <= 0.90) {
...
export function detectBullishPinBar(context: PatternContext): CandlePattern | null {
  ...
  // Basic requirements check
  const lowerWickValid = lowerWickRatio >= 0.50;
  const bodyRatioValid = bodyRatio <= 0.35;
  const closePosValid = closePosPct >= 0.50;
  ...
  if (lowerWickRatio < 0.50) {
    ...
    wickScore = 0.50; // medium
  } else if (lowerWickRatio >= 0.80 && lowerWickRatio <= 1.00) {
    wickScore = 0.75; // strong
  } else if (lowerWickRatio > 1.00) {
    wickScore = 1.0; // very strong
  }
  ...
  } else if (closePosPct >= 0.50 && closePosPct < 0.60) {
    closePosScore = 0.25; // weak
  } else if (closePosPct >= 0.60 && closePosPct < 0.75) {
    closePosScore = 0.50; // medium
  } else if (closePosPct >= 0.75 && closePosPct < 0.90) {
    closePosScore = 0.75; // strong
  } else if (closePosPct >= 0.90) {
  ...
  if (downTrendStrengthPct >= 0 && downTrendStrengthPct < 2.0) {
    ...
    trendScore = 0.50; // medium context
...
export function detectBearishPinBar(context: PatternContext): CandlePattern | null {
  ...
  const upperWickValid = upperWickRatio >= 0.50;
  const bodyRatioValid = bodyRatio <= 0.35;
  const closePosValid = closePosPctFromHigh >= 0.50;
  ...
  if (upperWickRatio < 0.50) {
    wickScore = 0.0; // invalid
  } else if (upperWickRatio >= 0.50 && upperWickRatio < 0.65) {
    wickScore = 0.25; // weak
  } else if (upperWickRatio >= 0.65 && upperWickRatio < 0.80) {
    wickScore = 0.50; // medium
  } else if (upperWickRatio >= 0.80 && upperWickRatio <= 1.00) {
  ...
  let closePosScore = 0.0;
  if (closePosPctFromHigh < 0.50) {
    closePosScore = 0.0; // invalid
  } else if (closePosPctFromHigh >= 0.50 && closePosPctFromHigh < 0.60) {
    closePosScore = 0.25; // weak
  } else if (closePosPctFromHigh >= 0.60 && closePosPctFromHigh < 0.75) {
    closePosScore = 0.50; // medium
  } else if (closePosPctFromHigh >= 0.75 && closePosPctFromHigh < 0.90) {
    closePosScore = 0.75; // strong
  } else if (closePosPctFromHigh >= 0.90) {
    closePosScore = 1.0; // very strong
  }
...
export function detectMorningStar(context: PatternContext): CandlePattern | null {
  ...
  // Recovery percentage
  const epsilon = 0.0001;
  const recoveryPct = c1BodySize > epsilon
    ? (c3.close - c1BodyMid) / c1BodySize
    : 0;
  ...
  const c1Condition = isBearishC1 && bodyRatio1 >= 0.40;
  ...
  const c3Condition = isBullishC3 && bodyRatio3 >= 0.40 && c3.close >= c1BodyMid;
  ...
  if (bodyRatio1 >= 0.30 && bodyRatio1 < 0.40) {
    firstBearScore = 0.25; // weak
  } else if (bodyRatio1 >= 0.40 && bodyRatio1 < 0.55) {
    firstBearScore = 0.50; // medium
  } else if (bodyRatio1 >= 0.55 && bodyRatio1 < 0.70) {
    firstBearScore = 0.75; // strong
  } else if (bodyRatio1 >= 0.70) {
  ...
  if (bodyRatio3 >= 0.30 && bodyRatio3 < 0.40) {
    thirdBullScore = 0.25; // weak
  } else if (bodyRatio3 >= 0.40 && bodyRatio3 < 0.55) {
    thirdBullScore = 0.50; // medium
  } else if (bodyRatio3 >= 0.55 && bodyRatio3 < 0.70) {
    thirdBullScore = 0.75; // strong
  } else if (bodyRatio3 >= 0.70) {
  ...
  } else if (recoveryPct >= 0 && recoveryPct < 0.20) {
    recoveryScore = 0.25; // weak
  } else if (recoveryPct >= 0.20 && recoveryPct < 0.50) {
    recoveryScore = 0.50; // medium
  } else if (recoveryPct >= 0.50 && recoveryPct <= 1.0) {
    recoveryScore = Math.min(1.0, 0.75 + (recoveryPct - 0.50) * 0.5); // strong, cap at 1.0
  }
  ...
  const strength = calculateWeightedStrength([
    ...
    { score: gapLikeScore, weight: 0.10 },
    { score: thirdBullScore, weight: 0.20 },
...
export function detectEveningStar(context: PatternContext): CandlePattern | null {
  ...
  const epsilon = 0.0001;
  const breakdownPct = c1BodySize > epsilon
    ? (c1BodyMid - c3.close) / c1BodySize
    : 0;
  ...
  const c1Condition = isBullishC1 && bodyRatio1 >= 0.40;
  ...
  const c3Condition = isBearishC3 && bodyRatio3 >= 0.40 && c3.close <= c1BodyMid;
  ...
  } else if (breakdownPct > 0 && breakdownPct < 0.20) {
    breakdownScore = 0.25; // weak
  } else if (breakdownPct >= 0.20 && breakdownPct < 0.50) {
    breakdownScore = 0.50; // medium
  } else if (breakdownPct >= 0.50 && breakdownPct <= 1.00) {
    breakdownScore = Math.min(1.0, 0.75 + (breakdownPct - 0.50) * 0.5); // strong, cap at 1.0
  }
  ...
  const strength = calculateWeightedStrength([
    ...
    { score: starScore, weight: 0.15 },
    { score: starAboveScore, weight: 0.10 },
    { score: thirdBearScore, weight: 0.20 },
...
export function detectThreeWhiteSoldiers(context: PatternContext): CandlePattern | null {
  ...
  const closeNearHigh = closeToHighPct1 <= 0.25 && closeToHighPct2 <= 0.25 && closeToHighPct3 <= 0.25;
  
  // C) Body strength
  const bodyStrengthValid = bodyRatio1 >= 0.40 && bodyRatio2 >= 0.40 && bodyRatio3 >= 0.40;
  ...
  const shadowsValid = 
    upperWickRatio1 <= 0.35 && upperWickRatio2 <= 0.35 && upperWickRatio3 <= 0.35 &&
    lowerWickRatio1 <= 0.35 && lowerWickRatio2 <= 0.35 && lowerWickRatio3 <= 0.35;
  
  // If any structural condition fails, return invalid
  if (!allBullish || !closeNearHigh || !bodyStrengthValid || !progressiveStructure || !shadowsValid) {
    return {
      index, // Return index of C3
  ...
  if (avgBodyRatio >= 0.30 && avgBodyRatio < 0.40) {
    bodyStrength = 0.25; // weak
  } else if (avgBodyRatio >= 0.40 && avgBodyRatio < 0.55) {
    bodyStrength = 0.50; // medium
  } else if (avgBodyRatio >= 0.55 && avgBodyRatio < 0.70) {
    bodyStrength = 0.75; // strong
  } else if (avgBodyRatio >= 0.70) {
  ...
  const strength = calculateWeightedStrength([
    ...
    { score: strengthCloseSequence, weight: 0.20 },
    { score: idealGapScore, weight: 0.10 },
    { score: trendScore, weight: 0.20 },
    { score: volumeScore, weight: 0.15 },
    { score: srScore, weight: 0.10 },
  ]);
...
export function detectThreeBlackCrows(context: PatternContext): CandlePattern | null {
  ...
  // If any structural condition fails, return invalid
  if (!allBearish || !closeNearLow || !bodyStrengthValid || !progressiveStructure || !shadowsValid) {
    return {
      index, // Return index of C3
  ...
  const strength = calculateWeightedStrength([
    { score: bodyStrength, weight: 0.25 },
    { score: declineSeqScore, weight: 0.20 },
    { score: idealGapScore, weight: 0.10 },
    { score: trendScore, weight: 0.20 },
    { score: volumeScore, weight: 0.15 },
    { score: srScore, weight: 0.10 },
...
export function detectBullishHarami(context: PatternContext): CandlePattern | null {
  ...
  const isBearishC1 = c1.close < c1.open;
  const isBullishC2 = c2.close > c2.open;
  const isDojiLikeC2 = Math.abs(c2.close - c2.open) / Math.max(c2.high - c2.low, 0.0001) <= 0.10;
  ...
  const epsilon = 0.0001;
  const insidePct = c1BodySize > epsilon
    ? (c1BodySize - c2BodySize) / c1BodySize
    : 0;
  ...
  const c2Condition = bodyRatio2 <= 0.30;
  ...
  // B) Insideness Ratio
  let insideScore = 0.0;
  if (insidePct >= 0.0 && insidePct < 0.20) {
    insideScore = 0.25; // weak
  } else if (insidePct >= 0.20 && insidePct < 0.50) {
    insideScore = 0.50; // medium
  } else if (insidePct >= 0.50 && insidePct < 0.80) {
    insideScore = 0.75; // strong
  } else if (insidePct >= 0.80) {
  ...
  const strength = calculateWeightedStrength([
    ...
    { score: colorScoreC2, weight: 0.10 },
    { score: trendScore, weight: 0.20 },
    { score: volumeScore, weight: 0.10 },
    { score: srScore, weight: 0.10 },
  ]);
...
export function detectBearishHarami(context: PatternContext): CandlePattern | null {
  ...
  const isBullishC1 = c1.close > c1.open;
  const isBearishC2 = c2.close < c2.open;
  const isDojiLikeC2 = bodyRatio2 <= 0.10;
...
export function detectPiercingLine(context: PatternContext): CandlePattern | null {
  ...
  const epsilon = 0.0001;
  const penetrationPct = c1BodySize > epsilon
    ? (c2.close - c1.close) / c1BodySize
    : 0;
  ...
  // If any structural condition fails, return invalid
  if (!c1Condition || !gapDownCondition || !penetrationCondition || !bullishCondition || !bodyCondition) {
    return {
      index, // Return index of C2
  ...
  } else if (penetrationPct >= 0.25 && penetrationPct < 0.50) {
    penetrationScore = 0.25; // weak
  } else if (penetrationPct >= 0.50 && penetrationPct < 0.75) {
    penetrationScore = 0.50; // medium
  ...
  } else if (bodyRatio2 >= 0.45 && bodyRatio2 < 0.60) {
    c2BodyScore = 0.50; // medium
  } else if (bodyRatio2 >= 0.60 && bodyRatio2 < 0.75) {
  ...
  // =========================
  const strength = calculateWeightedStrength([
    { score: bearStrengthC1, weight: 0.20 },
...
export function detectDarkCloudCover(context: PatternContext): CandlePattern | null {
  ...
  const penetrationPctDown = c1BullishBody > epsilon
    ? c2BearishBody / c1BullishBody
    : 0;
  ...
  // If any structural condition fails, return invalid
  if (!c1Condition || !gapUpCondition || !penetrationCondition || !bearishCondition || !bodyCondition) {
    return {
      index, // Return index of C2
  ...
  } else if (penetrationPctDown >= 0.25 && penetrationPctDown < 0.50) {
    penetrationScore = 0.25; // weak
  } else if (penetrationPctDown >= 0.50 && penetrationPctDown < 0.75) {
    penetrationScore = 0.50; // medium
  ...
  // =========================
  const strength = calculateWeightedStrength([
    { score: bullStrengthC1, weight: 0.20 },
    { score: penetrationScore, weight: 0.25 },
    { score: c2BodyScore, weight: 0.15 },
    { score: trendScore, weight: 0.15 },
    { score: volumeScore, weight: 0.15 },
...
export function detectBullishHaramiCross(context: PatternContext): CandlePattern | null {
  ...
  const c2Condition = bodyRatio2 <= 0.10;
  ...
  if (volumeMultiple2 >= 0.5 && volumeMultiple2 < 0.8) {
    ...
    volumeScore = 0.50; // neutral
  } else if (volumeMultiple2 >= 1.2 && volumeMultiple2 < 1.8) {
    volumeScore = 0.50; // participation
  } else if (volumeMultiple2 >= 1.8) {
  ...
  const strength = calculateWeightedStrength([
    ...
    { score: insidenessScore, weight: 0.20 },
    { score: trendScore, weight: 0.15 },
...
export function detectTweezersTop(context: PatternContext): CandlePattern | null {
  ...
  const epsilon = 0.0001;
  const highDiffPct = maxHigh > epsilon
    ? Math.abs(c2.high - c1.high) / maxHigh * 100
    : 100.0;
  ...
  } else if (highDiffPct >= 0.20 && highDiffPct <= 0.30) {
    highEqualityScore = 0.25; // weak
  } else if (highDiffPct >= 0.10 && highDiffPct < 0.20) {
    highEqualityScore = 0.50; // medium
  } else if (highDiffPct >= 0.05 && highDiffPct < 0.10) {
  ...
  if (rejectionPct2 < 0.20) {
    rejectionScoreC2 = 0.25; // weak
  } else if (rejectionPct2 >= 0.20 && rejectionPct2 < 0.40) {
    rejectionScoreC2 = 0.50; // medium
  } else if (rejectionPct2 >= 0.40 && rejectionPct2 < 0.60) {
    rejectionScoreC2 = 0.75; // strong
  } else if (rejectionPct2 >= 0.60) {
    rejectionScoreC2 = 1.0; // very strong
  }
  
  // C) C2 Body Direction & Size
  let colorScore = isBearishC2 ? 1.0 : 0.6;
  ...
  // =========================
  const strength = calculateWeightedStrength([
    { score: highEqualityScore, weight: 0.30 },
...
export function detectTweezersBottom(context: PatternContext): CandlePattern | null {
  ...
  const epsilon = 0.0001;
  const lowDiffPct = minLow > epsilon
    ? Math.abs(c2.low - c1.low) / minLow * 100
    : 100.0;
  ...
  const lowEqualityCondition = lowDiffPct <= 0.30;
  ...
  } else if (lowDiffPct >= 0.20 && lowDiffPct <= 0.30) {
    lowEqualityScore = 0.25; // weak
  } else if (lowDiffPct >= 0.10 && lowDiffPct < 0.20) {
    lowEqualityScore = 0.50; // medium
  } else if (lowDiffPct >= 0.05 && lowDiffPct < 0.10) {
  ...
  // C) C2 Body Direction
  let colorScore = isBullishC2 ? 1.0 : 0.6;
  ...
  // =========================
  const strength = calculateWeightedStrength([
    { score: lowEqualityScore, weight: 0.30 },
    { score: rejectionScoreC2, weight: 0.25 },
    { score: colorScore, weight: 0.05 },
    { score: trendScore, weight: 0.20 },
    { score: volumeScore, weight: 0.10 },
...
export function detectRisingThreeMethods(context: PatternContext): CandlePattern | null {
  ...
  // Validate candles
  if (c1.high === c1.low || c2.high === c2.low || c3.high === c3.low || c4.high === c4.low || c5.high === c5.low) {
    return null; // invalid candles
  }
  ...
  const smallTolerance = c1Range * 0.10; // 10% tolerance for low
  ...
  const epsilon = 0.0001;
  const pullbackDepthPct = c1CloseToLow > epsilon
    ? (c1.close - pullbackLowMin) / c1CloseToLow
    : 1.0;
  ...
  } else if (avgBigBody >= 0.45 && avgBigBody < 0.60) {
    bigBodyStrength = 0.50; // medium
  } else if (avgBigBody >= 0.60 && avgBigBody < 0.75) {
  ...
  } else if (pullbackRangePct >= 0.50 && pullbackRangePct < 0.75) {
    pullbackTightness = 0.50; // medium
  } else if (pullbackRangePct >= 0.25 && pullbackRangePct < 0.50) {
  ...
  } else if (pullbackDepthPct >= 0.50 && pullbackDepthPct < 0.75) {
    pullbackDepthScore = 0.50; // medium
  } else if (pullbackDepthPct >= 0.25 && pullbackDepthPct < 0.50) {
...
export function detectFallingThreeMethods(context: PatternContext): CandlePattern | null {
  ...
  const c1Range = c1.high - c1.low;
  const tolerance = c1Range * 0.05; // 5% tolerance
  const smallTolerance = c1Range * 0.10; // 10% tolerance for high
  ...
  const epsilon = 0.0001;
  const pullbackHeightPct = c1HighToClose > epsilon
    ? (pullbackHighMax - c1.close) / c1HighToClose
    : 1.0;
  ...
  } else if (pullbackHeightPct >= 0.50 && pullbackHeightPct < 0.75) {
    pullbackHeightScore = 0.50; // medium
  } else if (pullbackHeightPct >= 0.25 && pullbackHeightPct < 0.50) {
...
export function detectBullishMarubozu(context: PatternContext): CandlePattern | null {
  ...
  const distanceToNearestSRpct = calculateDistanceToSR(
    candle,
    context.nearestSupport,
    context.nearestResistance
  ...
  // B) Dominant body
  const bodyCondition = bodyRatio >= 0.80;
  
  // C) Very small shadows
  const combinedWickCondition = combinedWick <= 0.30;
  ...
  } else if (bodyRatio >= 0.70 && bodyRatio < 0.80) {
    bodyScore = 0.25; // weak
  } else if (bodyRatio >= 0.80 && bodyRatio < 0.90) {
    bodyScore = 0.50; // medium
  } else if (bodyRatio >= 0.90 && bodyRatio < 0.97) {
    bodyScore = 0.75; // strong
  ...
  let wickScore = 0.0;
  if (combinedWick > 0.30) {
    wickScore = 0.0; // invalid (already filtered)
  } else if (combinedWick >= 0.20 && combinedWick <= 0.30) {
    wickScore = 0.25; // weak
  } else if (combinedWick >= 0.10 && combinedWick < 0.20) {
    wickScore = 0.50; // medium
  } else if (combinedWick >= 0.05 && combinedWick < 0.10) {
    wickScore = 0.75; // strong
...
export function detectBullishBeltHold(context: PatternContext): CandlePattern | null {
  ...
  const lowerWickCondition = lowerWickRatio <= 0.10;
  
  // C) Large body
  const bodyCondition = bodyRatio >= 0.60;
  ...
  // B) Body Ratio Quality
  let bodyScore = 0.0;
  if (bodyRatio >= 0.60 && bodyRatio < 0.70) {
    bodyScore = 0.25; // weak
  } else if (bodyRatio >= 0.70 && bodyRatio < 0.80) {
    bodyScore = 0.50; // medium
  } else if (bodyRatio >= 0.80 && bodyRatio < 0.90) {
    bodyScore = 0.75; // strong
  } else if (bodyRatio >= 0.90) {
    bodyScore = 1.0; // very strong
  }
  ...
  if (upperWickRatio > 0.40) {
    upperWickScore = 0.0; // invalid
  } else if (upperWickRatio >= 0.30 && upperWickRatio <= 0.40) {
    upperWickScore = 0.25; // weak
  } else if (upperWickRatio >= 0.15 && upperWickRatio < 0.30) {
    upperWickScore = 0.50; // medium
  } else if (upperWickRatio >= 0.05 && upperWickRatio < 0.15) {
    upperWickScore = 0.75; // strong
  } else if (upperWickRatio < 0.05) {
...
export function detectBearishBeltHold(context: PatternContext): CandlePattern | null {
  ...
  const upperWickCondition = upperWickRatio <= 0.10;
  ...
  if (lowerWickRatio > 0.40) {
    lowerWickScore = 0.0; // invalid
  } else if (lowerWickRatio >= 0.30 && lowerWickRatio <= 0.40) {
    lowerWickScore = 0.25; // weak
  } else if (lowerWickRatio >= 0.15 && lowerWickRatio < 0.30) {
    lowerWickScore = 0.50; // medium
  } else if (lowerWickRatio >= 0.05 && lowerWickRatio < 0.15) {
    lowerWickScore = 0.75; // strong
  } else if (lowerWickRatio < 0.05) {
...
export function detectUpsideTasukiGap(context: PatternContext): CandlePattern | null {
  ...
  const gapFillPct = gapExists && gapSize > 0
    ? (c2.open - c3.low) / (c2.open - c1.high)
  ...
  const distanceToGapSupportPct = gapExists
    ? (c2.open - c3.low) / c2.open * 100
    : 100.0;
  ...
  const c1BullishCondition = isBullish1 && bodyRatio1 >= 0.40;
  ...
  // If any structural condition fails, return invalid
  if (!c1BullishCondition || !c2BullishCondition || !gapUpCondition || !c3BearishCondition || !gapNotFilledCondition || gapSize < 0.10) {
    return {
      index,
  ...
  } else if (gapSize >= 0.10 && gapSize < 0.20) {
    gapSizeScore = 0.25; // weak
  } else if (gapSize >= 0.20 && gapSize < 0.40) {
    gapSizeScore = 0.50; // medium
  } else if (gapSize >= 0.40 && gapSize < 0.70) {
    gapSizeScore = 0.75; // strong
  } else if (gapSize >= 0.70) {
  ...
  } else if (gapFillPct >= 0.60 && gapFillPct <= 0.80) {
    gapFillScore = 0.25; // weak
  } else if (gapFillPct >= 0.40 && gapFillPct < 0.60) {
    gapFillScore = 0.50; // medium
  } else if (gapFillPct >= 0.20 && gapFillPct < 0.40) {
    gapFillScore = 0.75; // strong
  } else if (gapFillPct < 0.20) {
    gapFillScore = Math.min(1.0, 0.75 + (0.20 - gapFillPct) * 1.25); // very strong, cap at 1.0
  }
  ...
  if (volumeMultiple3 < 0.9) {
    ...
    pullbackVolumeScore = 0.50; // neutral
  ...
  if (distanceToGapSupportPct < 0.10) {
    gapSupportScore = 1.0; // very strong support
  } else if (distanceToGapSupportPct >= 0.10 && distanceToGapSupportPct < 0.25) {
    gapSupportScore = 0.75; // strong support
  } else if (distanceToGapSupportPct >= 0.25 && distanceToGapSupportPct < 0.50) {
    gapSupportScore = 0.50; // medium support
  } else if (distanceToGapSupportPct >= 0.50 && distanceToGapSupportPct < 1.0) {
  ...
  const strength = calculateWeightedStrength([
    ...
    { score: trendScore, weight: 0.15 },
    { score: volumeScore1, weight: 0.10 },
    { score: volumeScore2, weight: 0.10 },
    { score: pullbackVolumeScore, weight: 0.05 },
    { score: gapSupportScore, weight: 0.10 },
  ]);
...
export function detectDownsideTasukiGap(context: PatternContext): CandlePattern | null {
  ...
  // Gap fill percentage (how much of the gap was filled by C3)
  const gapFillPct = gapExists && gapSize > 0 && (c1.low - c2.open) > 0
  ...
  const distanceToGapResistancePct = gapExists && c3.high > 0
    ? (c3.high - c2.open) / c3.high * 100
    : 100.0;
  ...
  const c1BearishCondition = isBearish1 && bodyRatio1 >= 0.40;
  ...
  // If any structural condition fails, return invalid
  if (!c1BearishCondition || !c2BearishCondition || !gapDownCondition || !c3BullishCondition || !gapNotFilledCondition || gapSize < 0.10) {
    return {
      index,
  ...
  if (distanceToGapResistancePct < 0.10) {
    gapResistanceScore = 1.0; // very strong resistance
  } else if (distanceToGapResistancePct >= 0.10 && distanceToGapResistancePct < 0.25) {
    gapResistanceScore = 0.75; // strong resistance
  } else if (distanceToGapResistancePct >= 0.25 && distanceToGapResistancePct < 0.50) {
    gapResistanceScore = 0.50; // medium resistance
  } else if (distanceToGapResistancePct >= 0.50 && distanceToGapResistancePct < 1.0) {
  ...
  const strength = calculateWeightedStrength([
    ...
    { score: volumeScore2, weight: 0.10 },
    { score: pullbackVolumeScore, weight: 0.05 },
    { score: gapResistanceScore, weight: 0.10 },
  ]);
...
export function detectBullishKicking(context: PatternContext): CandlePattern | null {
  ...
  const c1BodyCondition = bodyRatio1 >= 0.80;
  const c1WickCondition = combinedWick1 <= 0.20;
  
  // B) C2 bullish with almost no wicks
  const c2BullishCondition = isBullish2;
  const c2BodyCondition = bodyRatio2 >= 0.80;
  const c2WickCondition = combinedWick2 <= 0.20;
  
  // C) Gap between the two candles
  const gapCondition = gapExists;
  
  // If any structural condition fails, return invalid
  if (!c1BearishCondition || !c1BodyCondition || !c1WickCondition || 
      !c2BullishCondition || !c2BodyCondition || !c2WickCondition || 
      !gapCondition) {
    return {
      index,
  ...
  let bodyScore1 = 0.0;
  if (bodyRatio1 >= 0.80 && bodyRatio1 < 0.90) {
    bodyScore1 = 0.50; // medium
  } else if (bodyRatio1 >= 0.90 && bodyRatio1 < 0.97) {
  ...
  let bodyScore2 = 0.0;
  if (bodyRatio2 >= 0.80 && bodyRatio2 < 0.90) {
    bodyScore2 = 0.50; // medium
  } else if (bodyRatio2 >= 0.90 && bodyRatio2 < 0.97) {
  ...
  if (gapSizePct < 0.15) {
    ...
    gapSizeScore = 0.50; // medium
  } else if (gapSizePct >= 0.35 && gapSizePct < 0.60) {
    gapSizeScore = 0.75; // strong
  } else if (gapSizePct >= 0.60) {
    gapSizeScore = 1.0; // very strong
  }
  ...
  } else if (combinedWick1 <= 0.10) {
    wickScore1 = 0.75; // strong
  } else if (combinedWick1 <= 0.20) {
    wickScore1 = 0.50; // acceptable
  }
  ...
  } else if (combinedWick2 <= 0.10) {
    wickScore2 = 0.75; // strong
  } else if (combinedWick2 <= 0.20) {
    wickScore2 = 0.50; // acceptable
  }
  ...
  // =========================
  const strength = calculateWeightedStrength([
    { score: bodyScore1, weight: 0.20 },
    { score: bodyScore2, weight: 0.20 },
    { score: gapSizeScore, weight: 0.25 },
    { score: trendScore, weight: 0.15 },
    { score: volumeScore2, weight: 0.10 },
    { score: srScore, weight: 0.10 },
...
export function detectBearishKicking(context: PatternContext): CandlePattern | null {
  ...
  // If any structural condition fails, return invalid
  if (!c1BullishCondition || !c1BodyCondition || !c1WickCondition || 
      !c2BearishCondition || !c2BodyCondition || !c2WickCondition || 
      !gapCondition) {
    return {
      index,
...
export function detectConcealingBabySwallow(context: PatternContext): CandlePattern | null {
  ...
  if (gapSizePct < 0.10) {
    gapScore = 0.25; // weak
  } else if (gapSizePct >= 0.10 && gapSizePct < 0.25) {
    gapScore = 0.50; // medium
  } else if (gapSizePct >= 0.25 && gapSizePct < 0.50) {
    gapScore = 0.75; // strong
  } else if (gapSizePct >= 0.50) {
  ...
  // Body ratio score
  let c4BodyScore = 0.0;
  if (bodyRatio4 >= 0.45 && bodyRatio4 < 0.60) {
    c4BodyScore = 0.50; // medium
  } else if (bodyRatio4 >= 0.60 && bodyRatio4 < 0.75) {
  ...
  const strength = calculateWeightedStrength([
    { score: trendScore, weight: 0.20 },
    { score: gapScore, weight: 0.20 },
    { score: babyInsideScore, weight: 0.20 },
...
export function detectBullishBreakaway(context: PatternContext): CandlePattern | null {
  ...
  const penetrationC2Pct = c2BodySize > 0 
    ? (c5.close - c2.close) / (c2.open - c2.close) 
    : 0;
  
  const c1BodySize = Math.abs(c1.open - c1.close);
  const penetrationC1Pct = c1BodySize > 0 
    ? (c5.close - c1.close) / (c1.open - c1.close) 
    : 0;
  ...
  const c2Condition = isBearish2 && gapExists && bodyRatio2 >= 0.30;
  ...
  const c5Condition = isBullish5 && 
    bodyRatio5 >= 0.45 && 
    penetrationC2; // at least above C2 midpoint
  ...
  const smallBodyScore = avgBodySmall <= 0.20 ? 1.0 : 
                        avgBodySmall <= 0.30 ? 0.75 : 
                        avgBodySmall <= 0.35 ? 0.50 : 0.25;
  
  const tightRangeScore = clusterRangePct <= 0.50 ? 1.0 :
                          clusterRangePct <= 0.70 ? 0.75 :
                          clusterRangePct <= 0.90 ? 0.50 : 0.25;
  ...
  if (bodyRatio5 >= 0.45 && bodyRatio5 < 0.55) {
    c5BodyScore = 0.50; // medium
  } else if (bodyRatio5 >= 0.55 && bodyRatio5 < 0.70) {
    c5BodyScore = 0.75; // strong
  } else if (bodyRatio5 >= 0.70) {
  ...
  } else if (penetrationC2 && penetrationC2Pct >= 0.50) {
    penetrationScore = 0.75; // strong - deep into C2
  } else if (penetrationC2) {
    penetrationScore = 0.50; // medium - above C2 midpoint
  }
...
export function detectBearishBreakaway(context: PatternContext): CandlePattern | null {
  ...
  const c2BodySize = Math.abs(c2.open - c2.close);
  const penetrationC2Pct = c2BodySize > 0 
    ? (c2.close - c5.close) / (c2.close - c2.open) 
    : 0;
  
  const c1BodySize = Math.abs(c1.open - c1.close);
  const penetrationC1Pct = c1BodySize > 0 
    ? (c1.close - c5.close) / (c1.close - c1.open) 
    : 0;
  ...
  const c2Condition = isBullish2 && gapExists && bodyRatio2 >= 0.30;
  ...
  const c5Condition = isBearish5 && 
    bodyRatio5 >= 0.45 && 
    penetrationC2; // at least below C2 midpoint
  ...
  if (penetrationC1) {
    ...
    penetrationScore = 0.50; // medium - below C2 midpoint
  }
...
export function detectUpsideGapTwoCrows(context: PatternContext): CandlePattern | null {
  ...
  const c1BodyHigh = Math.max(c1.open, c1.close);
  const c1BodySize = c1BodyHigh - c1BodyLow;
  const penetrationPct = c1BodySize > 0
    ? (c1BodyHigh - c3.close) / c1BodySize
    : 0;
  ...
  const c2Condition = isBearish2 && gapExists && 
    bodyRatio2 >= 0.20 && bodyRatio2 <= 0.50;
  ...
  // =========================
  const strength = calculateWeightedStrength([
    { score: c1StrengthScore, weight: 0.20 },
    { score: gapSizeScore, weight: 0.20 },
    { score: downClusterScore, weight: 0.20 },
    { score: penetrationScore, weight: 0.20 },
    { score: trendScore, weight: 0.10 },
    { score: volumeScore2_3, weight: 0.10 },
  ]);
...
export function detectDownsideGapThreeMethods(context: PatternContext): CandlePattern | null {
  ...
  if (bodyRatio2 >= 0.35 && bodyRatio2 < 0.50) {
    bearStrengthC2 = 0.50; // medium
  } else if (bodyRatio2 >= 0.50 && bodyRatio2 < 0.70) {
    bearStrengthC2 = 0.75; // strong
  } else if (bodyRatio2 >= 0.70) {
    bearStrengthC2 = 1.0; // very strong
  }
  
  // E) C4 bearish strength score
  let bearStrengthC4 = 0.0;
  if (bodyRatio4 >= 0.35 && bodyRatio4 < 0.50) {
    bearStrengthC4 = 0.50; // medium
  } else if (bodyRatio4 >= 0.50 && bodyRatio4 < 0.70) {
    bearStrengthC4 = 0.75; // strong
  } else if (bodyRatio4 >= 0.70) {
  ...
  const strength = calculateWeightedStrength([
    { score: trendScore, weight: 0.20 },
    { score: gapSizeScore, weight: 0.20 },
    { score: clusterTightScore, weight: 0.20 },
    { score: bearStrengthC2, weight: 0.15 },
    { score: bearStrengthC4, weight: 0.15 },
    { score: volumeScore24, weight: 0.10 },
  ]);
...
export function detectOnNeck(context: PatternContext): CandlePattern | null {
  ...
  const neckCondition = neckDiffPct <= 0.30;
  ...
  } else if (neckDiffPct >= 0.20 && neckDiffPct <= 0.30) {
    neckTouchScore = 0.25; // weak
  } else if (neckDiffPct >= 0.10 && neckDiffPct < 0.20) {
    neckTouchScore = 0.50; // medium
  } else if (neckDiffPct >= 0.05 && neckDiffPct < 0.10) {
  ...
  // =========================
  const strength = calculateWeightedStrength([
    { score: bearStrengthC1, weight: 0.30 },
    { score: smallBodyScoreC2, weight: 0.15 },
    { score: neckTouchScore, weight: 0.20 },
    { score: trendScore, weight: 0.20 },
    { score: volumeScore, weight: 0.15 },
  ]);
...
export function detectInNeck(context: PatternContext): CandlePattern | null {
  ...
  const closePosPctInBody = c1Range > 0
    ? (c2.close - c1.low) / c1Range
    : 1.0;
  ...
  const positionCondition = close2AboveLow1 && close2InLowerHalf && closePosPctInBody > 0 && closePosPctInBody <= 0.5;
  ...
  const strength = calculateWeightedStrength([
    { score: bearStrengthC1, weight: 0.30 },
    { score: smallBodyScoreC2, weight: 0.15 },
    { score: closePosScore, weight: 0.20 },
    { score: trendScore, weight: 0.20 },
    { score: volumeScore, weight: 0.15 },
  ]);
...
export function detectThrusting(context: PatternContext): CandlePattern | null {
  ...
  const penetrationPct = c1MidToLow > 0
    ? (c2.close - c1.low) / c1MidToLow
    : 1.0;
  ...
  const c2Condition = isBullish2 && gapDownExists && 
    bodyRatio2 >= 0.25 && bodyRatio2 <= 0.50;
  ...
  if (bodyRatio2 >= 0.25 && bodyRatio2 < 0.30) {
    bodySizeC2Score = 0.50; // medium (low end of range)
  } else if (bodyRatio2 >= 0.30 && bodyRatio2 < 0.40) {
    bodySizeC2Score = 0.75; // strong (middle range)
  } else if (bodyRatio2 >= 0.40 && bodyRatio2 <= 0.50) {
  ...
  if (penetrationPct > 1.0) {
    ...
    penetrationScore = 0.50; // medium
  } else if (penetrationPct >= 0.50 && penetrationPct <= 1.0) {
    penetrationScore = 0.75 + (penetrationPct - 0.50) * 0.5; // strong to very strong, cap at 1.0
  }
  ...
  if (volumeMultiple2 >= 0.8 && volumeMultiple2 < 1.2) {
    ...
    volumeScoreC2 = 0.50; // medium - elevated but ok
    ...
    volumeScoreC2 = 0.50; // medium - low volume
  }
...
export function detectMatchingLow(context: PatternContext): CandlePattern | null {
  ...
  // A) C1: bearish
  const c1Condition = isBearish1 && bodyRatio1 >= 0.30;
  
  // B) C2: bearish or doji-like
  const c2Condition = isBearishOrDoji2 && bodyRatio2 <= 0.50;
  
  // C) Lows of C1 and C2 nearly equal
  const lowMatchCondition = lowDiffPct <= 0.30;
  ...
  if (volumeMultiple2 >= 0.7 && volumeMultiple2 < 1.2) {
    ...
    volumeScore2 = 0.50; // medium - elevated (may indicate capitulation)
  } else if (volumeMultiple2 >= 1.8 && volumeMultiple2 < 2.5) {
    ...
    volumeScore2 = 0.50; // medium - low volume
  }
  
  // E) C2 body score (smaller is better for support test)
  let bodyScore2 = 0.0;
  if (bodyRatio2 <= 0.50 && bodyRatio2 > 0.35) {
    bodyScore2 = 0.50; // medium
  } else if (bodyRatio2 <= 0.35 && bodyRatio2 > 0.20) {
    bodyScore2 = 0.75; // strong
  } else if (bodyRatio2 <= 0.20) {
  ...
  const strength = calculateWeightedStrength([
    ...
    { score: bearStrengthC1, weight: 0.20 },
    { score: lowMatchScore, weight: 0.30 },
    { score: volumeScore2, weight: 0.15 },
    { score: bodyScore2, weight: 0.10 },
  ]);
...
export function detectStickSandwich(context: PatternContext): CandlePattern | null {
  ...
  const closeMatchPct = avgClose > 0
    ? Math.abs(c3.close - c1.close) / avgClose * 100
    : 100.0;
  ...
  const sandwichCondition = closeMatchPct <= 0.30 && 
    close1BelowClose2 && 
    close3BelowClose2;
  ...
  if (bodyRatio1 >= 0.35 && bodyRatio1 < 0.45) {
    bodyScore1 = 0.50; // medium
  } else if (bodyRatio1 >= 0.45 && bodyRatio1 < 0.60) {
    bodyScore1 = 0.75; // strong
  } else if (bodyRatio1 >= 0.60) {
    bodyScore1 = 1.0; // very strong
  } else {
  ...
  if (bodyRatio3 >= 0.35 && bodyRatio3 < 0.45) {
    bodyScore3 = 0.50; // medium
  } else if (bodyRatio3 >= 0.45 && bodyRatio3 < 0.60) {
    bodyScore3 = 0.75; // strong
  } else if (bodyRatio3 >= 0.60) {
  ...
  if (closeMatchPct > 0.30) {
    closeMatchScore = 0.0; // invalid (already filtered)
  } else if (closeMatchPct >= 0.20 && closeMatchPct <= 0.30) {
    closeMatchScore = 0.25; // weak
  } else if (closeMatchPct >= 0.10 && closeMatchPct < 0.20) {
    closeMatchScore = 0.50; // medium
  } else if (closeMatchPct >= 0.05 && closeMatchPct < 0.10) {
  ...
  if (volumeMultiple3 >= 0.8 && volumeMultiple3 < 1.2) {
    ...
    if (volumeMultiple3 < volumeMultiple1) {
      ...
      volumeScore3 = 0.50; // medium - low but not worse than C1
    }
  }
...
export function detectLadderBottom(context: PatternContext): CandlePattern | null {
  ...
  const c1Condition = isBearish1 && bodyRatio1 >= 0.30;
  const c2Condition = isBearish2 && bodyRatio2 >= 0.30 && low2BelowLow1;
  const c3Condition = isBearish3 && bodyRatio3 >= 0.30 && low3BelowLow2;
  
  // B) C4: small real body (doji/spinning top)
  const c4Condition = bodyRatio4 <= 0.25;
  
  // C) C5: strong bullish candle
  const c5Condition = isBullish5 && bodyRatio5 >= 0.40 && close5AboveClose4;
  ...
  if (bodyRatio5 >= 0.40 && bodyRatio5 < 0.50) {
    bodyStrengthC5 = 0.50; // medium
  } else if (bodyRatio5 >= 0.50 && bodyRatio5 < 0.65) {
...
export function detectLadderTop(context: PatternContext): CandlePattern | null {
  ...
  const c1Condition = isBullish1 && bodyRatio1 >= 0.30;
  const c2Condition = isBullish2 && bodyRatio2 >= 0.30 && high2AboveHigh1;
  const c3Condition = isBullish3 && bodyRatio3 >= 0.30 && high3AboveHigh2;
  
  // B) C4: small real body (doji/spinning top)
  const c4Condition = bodyRatio4 <= 0.25;
  
  // C) C5: strong bearish candle
  const c5Condition = isBearish5 && bodyRatio5 >= 0.40 && close5BelowClose4;
